...
const showImages = (images) => {
  ...
  images.forEach(image => {
    ...
    <img class="img-fluid img-thumbnail" onclick=selectItem(event,"${image.webformatURL}") src="${image.webformatURL}" alt="${image.tags}" title="${image.tags}">
    </div>
...
const createSlider = () => {
  ...
  if(duration > 999){
    ...
    alert(`Please put duration value at least 1000 (Here input value is in mili second)`);
...
//function for input can not match with the image's database.
const errorTogglePopUp = () => {
  document.getElementById("popup-2").classList.toggle("active");
  const errorInfo = `
      <img class="sorry-img" src="images/image-not-found.png">
